...
## ຂັ້ນຕອນທີ 5: ຕັ້ງຄ່າ AI Assistant (Google Gemini)

ໜ້າຮ້ານມີ Chat Widget ຮູບ 💬 ລອຍຢູ່ມຸມຂວາລຸ່ມ ໃຫ້ລູກຄ້າຖາມກ່ຽວກັບສິນຄ້າໄດ້. ລະບົບອອກແບບໃຫ້ AI ຮູ້ຈັກສະເພາະສິນຄ້າທີ່ມີຈິງໃນ `products.json` ເທົ່ານັ້ນ (ບໍ່ແນະນຳສິນຄ້ານອກຮ້ານ).

**ສຳຄັນ:** Gemini API Key ຫ້າມໃສ່ໃນ `config.js` ຫຼືໂຄ້ດ frontend ໂດຍກົງເດັດຂາດ ເພາະ repo ຂອງທ່ານເປັນ public — ໃຜກໍ່ເປີດເບິ່ງໄຟລ໌ໄດ້ ແລະ ຈະເອົາ key ໄປໃຊ້ຟຣີແທນທ່ານ. ຕ້ອງມີຊັ້ນ proxy ຄັ່ນກາງເກັບ key ໄວ້ສະເໝີ.

### 5.1 ຂໍ Gemini API Key
1. ໄປ https://aistudio.google.com/app/apikey (ຕ້ອງ login ດ້ວຍບັນຊີ Google)
2. ກົດ **Create API key** → ຄັດລອກ key ໄວ້ (ຂຶ້ນຕົ້ນດ້ວຍ `AIza...`)
3. Gemini API ມີ free tier ໃຫ້ໃຊ້ໄດ້ຈຳນວນໜຶ່ງຕໍ່ນາທີ/ຕໍ່ມື້ ໂດຍບໍ່ເສຍຄ່າ ເໝາະສຳລັບຮ້ານຂະໜາດນ້ອຍ-ກາງ — ຖ້າໃຊ້ຫຼາຍຄວນເບິ່ງລາຄາທີ່ https://ai.google.dev/gemini-api/docs/pricing

### 5.2 ຖ້າ deploy ຢູ່ GitHub Pages (ໃຊ້ Cloudflare Worker ເປັນ proxy) ← ກໍລະນີຂອງທ່ານ

GitHub Pages ເປັນ static hosting ລ້ວນໆ, ບໍ່ຮອງຮັບການຮັນໂຄ້ດຝັ່ງ server, ຈຶ່ງບໍ່ສາມາດເກັບ API Key ໄວ້ໄດ້ໂດຍກົງ. ວິທີແກ້ຄື ໃຊ້ **Cloudflare Workers** (ຟຣີ, ບໍ່ຕ້ອງໃສ່ບັດເຄຣດິດ) ເປັນ server ນ້ອຍໆຄັ່ນກາງ — GitHub Pages ຍັງໃຊ້ຄືເກົ່າ 100%, ມີແຕ່ Chat Widget ທີ່ຈະໄປຄຸຍກັບ Worker ນີ້ແທນ.

1. ສະໝັກບັນຊີຟຣີ https://dash.cloudflare.com/sign-up
2. ໃນ Dashboard → **Workers & Pages** → **Create** → **Create Worker**
3. ຕັ້ງຊື່ Worker (ເຊັ່ນ `gemini-chat`) → **Deploy** (ຈະໄດ້ໂຄ້ດຕົວຢ່າງ default ກ່ອນ)
4. ກົດ **Edit code** → ລຶບໂຄ້ດຕົວຢ່າງທັງໝົດອອກ → ຄັດລອກເນື້ອຫາທັງໝົດຈາກໄຟລ໌ `cloudflare-worker/gemini-chat-worker.js` ວາງແທນ → ກົດ **Deploy** ອີກຄັ້ງ
5. ໄປ **Settings → Variables and Secrets** ຂອງ Worker ນີ້ → ເພີ່ມ:
   - `GEMINI_API_KEY` (ເລືອກປະເພດ **Secret**) = key ຈາກຂໍ້ 5.1
   - `ALLOWED_ORIGIN` (ປະເພດ Text) = `https://YOUR_GITHUB_USERNAME.github.io` (ຕ້ອງກົງກັບ URL ຮ້ານທ່ານແທ້ໆ, ບໍ່ໃສ່ `/` ທ້າຍ) — ອັນນີ້ຈຳກັດບໍ່ໃຫ້ເວັບອື່ນມາເອີ້ນໃຊ້ Worker ຂອງທ່ານໄດ້
   - `GEMINI_MODEL` (ບໍ່ບັງຄັບ) = `gemini-2.5-flash` ຫຼື model ອື່ນ
   - ບັນທຶກ ແລ້ວ **redeploy** Worker ອີກຄັ້ງໃຫ້ env var ມີຜົນ
6. ຄັດລອກ URL ຂອງ Worker ຈາກໜ້າ Dashboard (ຮູບແບບ `https://gemini-chat.YOUR-SUBDOMAIN.workers.dev`)
7. ເປີດ `config.js` → ແກ້ `GEMINI_CHAT_ENDPOINT` ໃຫ້ເປັນ URL ນັ້ນ (ໃສ່ແບບເຕັມ, ບໍ່ແມ່ນ path ຍ່ອຍ) → commit ຂຶ້ນ GitHub
8. ລໍຖ້າ GitHub Pages build ໃໝ່ (~1 ນາທີ) ແລ້ວທົດລອງເປີດຮ້ານ → ກົດ Chat Bubble → ລອງຖາມ

### 5.3 ຖ້າ deploy ຢູ່ Vercel ຫຼື Netlify (ບໍ່ຕ້ອງໃຊ້ Cloudflare Worker)

ຖ້າພາຍຫຼັງທ່ານປ່ຽນຈາກ GitHub Pages ໄປໃຊ້ Vercel ຫຼື Netlify ແທນ (ເຊື່ອມກັບ GitHub repo ດຽວກັນ, auto-deploy ທຸກຄັ້ງທີ່ push), ໂປຣເຈັກນີ້ມີ serverless function ພ້ອມໃຊ້ຢູ່ແລ້ວ ງ່າຍກວ່າ Cloudflare Worker:

**Vercel:**
1. Project → **Settings → Environment Variables** → ເພີ່ມ `GEMINI_API_KEY` (ແລະ `GEMINI_MODEL` ຖ້າຕ້ອງການ)
2. Redeploy ໜຶ່ງຄັ້ງ
3. ແກ້ `config.js` → `GEMINI_CHAT_ENDPOINT: '/api/gemini-chat'`
4. Vercel ຈະກວດພົບ `api/gemini-chat.js` ອັດຕະໂນມັດ

**Netlify:**
1. **Site settings → Environment variables** → ເພີ່ມ `GEMINI_API_KEY` (ແລະ `GEMINI_MODEL` ຖ້າຕ້ອງການ)
2. ແກ້ `config.js` → `GEMINI_CHAT_ENDPOINT: '/.netlify/functions/gemini-chat'`
3. Trigger deploy ໃໝ່ — Netlify ຈະກວດພົບ `netlify/functions/gemini-chat.js` ອັດຕະໂນມັດ

### 5.4 ທົດສອບ
ເປີດໜ້າຮ້ານ → ກົດ Chat Bubble ມຸມຂວາລຸ່ມ → ລອງພິມຄຳຖາມ ເຊັ່ນ "MacBook ທີ່ຖືກທີ່ສຸດຕອນນີ້ລາຄາເທົ່າໃດ?" — ຖ້າຂຶ້ນຂໍ້ຄວາມ error ໃຫ້ເປີດ Console (F12) ເບິ່ງລາຍລະອຽດ, ສ່ວນຫຼາຍມາຈາກ: (1) `GEMINI_CHAT_ENDPOINT` ຍັງເປັນຄ່າ placeholder ຢູ່ (2) ຍັງບໍ່ໄດ້ຕັ້ງ `GEMINI_API_KEY` (3) `ALLOWED_ORIGIN` ບໍ່ກົງກັບ URL ຈິງຂອງຮ້ານ (ກໍລະນີໃຊ້ Cloudflare Worker)

### 5.5 ຄວາມປອດໄພ ແລະ ຄ່າໃຊ້ຈ່າຍ
- `ALLOWED_ORIGIN` (Cloudflare Worker) ຫຼື CORS default (Vercel/Netlify) ຈຳກັດບໍ່ໃຫ້ domain ອື່ນມາເອີ້ນໃຊ້ endpoint ຂອງທ່ານໄດ້ຟຣີໆ
- ຂໍ້ມູນສິນຄ້າ (`products`) ຖືກສົ່ງຈາກ browser ໄປໃຫ້ proxy ທຸກຄັ້ງທີ່ຖາມ — ອັນນີ້ໂອເຄເພາະຂໍ້ມູນນີ້ເປັນ public ຢູ່ແລ້ວ (ຄືກັນກັບທີ່ raw.githubusercontent.com ໃຫ້ໃຜກໍ່ອ່ານໄດ້)
- ຖ້າຮ້ານມີສິນຄ້າຫຼາຍຫຼາຍຮ້ອຍລາຍການ, proxy ຈະຕັດເຫຼືອ 300 ລາຍການທຳອິດເພື່ອຄວບຄຸມຂະໜາດ ແລະ ຄ່າໃຊ້ຈ່າຍ token
- Cloudflare Workers free tier ໃຫ້ 100,000 requests/ມື້ ຟຣີ — ພຽງພໍຫຼາຍສຳລັບຮ້ານຂະໜາດນ້ອຍ-ກາງ
